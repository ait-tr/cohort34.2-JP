...
public class UserController {
    ...
    // /users/{name}/{id}
    //method(@PathVariable  Long id, @PathVariable  String name)
    // /users/andy/22


    //@PutMapping("/users/{id}")
    @PutMapping("/{id}")
    public UserResponseDTO updateUserById(@PathVariable  Long id, @RequestBody UserRequestDTO user){
        return service.userUpdate(user, id);
    }
